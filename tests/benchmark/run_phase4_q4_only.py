"""Rerun only Phase 4 Q4 ([UNCERTAIN] marker) — quick standalone check
without sitting through the full benchmark."""
...
q4_uncertain_marker_appears = _mod.q4_uncertain_marker_appears
...
def main() -> int:
    ...
    cfg_q4 = cfg.model_copy(deep=True)
    cfg_q4.compaction.mask_uncertain = True

    print("Q4 ([UNCERTAIN] marker) standalone rerun", flush=True)
    t0 = time.time()
    ok, msg = q4_uncertain_marker_appears(Orchestrator(cfg_q4), uid)
    dur = time.time() - t0
