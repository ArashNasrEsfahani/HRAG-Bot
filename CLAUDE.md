...
**P11** (`retrieval.reflection_mode` = `off`|`regex`|`hybrid`, default hybrid; live-switchable via `GET/POST /api/config`) — reflective/opinion personal questions ("what do you think about me?"): broaden retrieval with profile terms + render `prompts/answer_personal_reflect.md` (registry `render_personal_reflect`). Detection is two-tier asymmetric: `intent.is_reflective_strict()` (high precision, EN + ZWNJ-FA + Finglish; only it or the LLM judge may coerce non-PERSONAL→PERSONAL), `is_reflective_query()` (recall tier, within already-PERSONAL turns), `has_reflective_anchor()` (corroboration gate — an opinion cue / `me`·`my`·`myself` / "who I am"; bare subject `i` is NOT an anchor — so a lone false LLM "yes" on a neutral message like "Ok i want to test you" can't hijack the turn into a self-portrait). Hybrid LLM signal from `combined_preflight.reflective` or lazy `ReflectiveClassifier` (`prompts/reflective_check.md`); consulted only when an anchor is present. `answer_personal_reflect.md` frames document excerpts as library subject-matter, NOT facts about the user (facts come only from profile + memories). Event `reflective_check {mode,strict,loose,llm,reflective}`.
